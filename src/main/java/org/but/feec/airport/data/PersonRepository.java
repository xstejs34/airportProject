...
public class PersonRepository {
    ...
    public void createPerson(PersonCreateView personCreateView) {
        String insertPersonSQL =
                "WITH ins1 AS ( INSERT INTO airport_sys.customer (given_name,family_name,date_of_birth,diet,username, password) " +
                        ...
                        " ins3 AS ( SELECT id_country FROM airport_sys.country WHERE country= ?)," +
    ...
    public void deletePerson(PersonDeleteView personDeleteView) {
        String deletePersonSQL = "with customerToDelete as ( DELETE FROM airport_sys.customer e WHERE id_customer= ? RETURNING id_customer)," +
                " hasAddressToDelete as (DELETE FROM airport_sys.customer_has_address p WHERE p.id_customer=(select id_customer from customerToDelete) RETURNING id_address)," +
                " addressToDelete as (DELETE FROM airport_sys.address a WHERE a.id_address=(select id_address from hasAddressToDelete) )" +
                " DELETE FROM airport_sys.contact c where c.id_customer=(select id_customer from customerToDelete)";

        String checkIfExists = "SELECT username FROM airport_sys.customer e WHERE e.id_customer =?";
        try (Connection connection = DataSourceConfig.getConnection();
             PreparedStatement preparedStatement = connection.prepareStatement(deletePersonSQL, Statement.RETURN_GENERATED_KEYS)) {

            preparedStatement.setInt(1, personDeleteView.getId());

            try {
                connection.setAutoCommit(false);
                try (PreparedStatement ps = connection.prepareStatement(checkIfExists)){
                    ps.setInt(1, personDeleteView.getId());
                    ps.execute();
                } catch (SQLException e) {
                    throw new DataAccessException("This person for delete does not exist.");
                }
